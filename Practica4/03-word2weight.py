...
#---------------------------第一步:读取数据-------------------------------
word = [] #记录关键词
f = open("all-data-key_v1.txt", encoding='utf-8')            
line = f.readline()           
while line:
    #print line
    line = line.replace("\n", "") #过滤换行
...
#--------------------------第三步  TXT文件写入--------------------------
res = open("word_word_weight_v1.txt", "a+", encoding='utf-8')
i = 0
while i<len(word):
    w1 = word[i]
    j = 0
...
#--------------------------第四步  CSV文件写入--------------------------
c = open("word-word-weight_v1.csv","w", encoding='utf-8', newline='')    #解决空行
